...
<table class="list_products">
	...
	<c:if test="${products.size() == 0}">
		<h3 style="padding: 10px 0 500px 0; text-align: center">No products were found.</h3>
	</c:if>
...
<c:if test="${param.index > 1}">
	...
	<c:forEach begin="1" end="${endPage}" var="i">
		<a href="<c:url value='/search?index=${i}'/>"
			<c:if test="${param.index == i}">class="active"</c:if>>${i}</a>
	</c:forEach>
	<c:if test="${param.index < endPage}">
	<a href="<c:url value='/search?index=${param.index+1}&search=${text}'/>">>></a>
	</c:if>
</div>
